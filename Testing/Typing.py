...
last_update = pygame.time.get_ticks() # 마지막으로 글자가 추가된 시간
speed = 50 # 150ms(=0.15초)마다 한 글자씩
...
while running:
    ...
    now = pygame.time.get_ticks() # 현재 시간 체크해서 지났으면 텍스트 추가
    
    if now - last_update > speed and count < len(FullText):
    ...
    for c in currentText:
        test_line = currentLine + c

        if font.size(test_line)[0] > ScreenSize[0]:
    ...
    for i, line in enumerate(lines):
        ...
        screen.blit(text,(0,0 + (i * line_height)))
